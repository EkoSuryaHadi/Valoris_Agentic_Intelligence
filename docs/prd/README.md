...
| MVP-C change management workspace | Implemented | `packages/api/src/change.js`, `packages/api/src/server.js`, `apps/web/index.html`, `apps/web/app.js`; weighted exposure is advisory and incorporation requires an approved change plus authorized human manager |
| MVP-C cash flow control workspace | Implemented | `packages/api/src/risk-cash.js`, `packages/api/src/server.js`, `apps/web/index.html`, `apps/web/app.js`; aligned planned/actual series provide visible variance and cumulative forecast without auto-action |
| MVP-C risk and agent finding review workspace | Implemented | `packages/api/src/risk-cash.js`, `packages/api/src/agent-import.js`, `packages/api/src/server.js`, `apps/web/index.html`, `apps/web/app.js`; weighted risks and evidence-linked findings require explicit human disposition |
| MVP-D production readiness | Implemented | `docs/implementation/PHASE17_PRODUCTION_READINESS.md`; release gates, environment checklist, demo seed plan, and Definition of Ready/Done |
...
| Phase 18 staging validation | Local staging validation complete; deployment verification pending | `docs/implementation/PHASE18_STAGING_VALIDATION.md`; 96 automated tests, PostgreSQL seed, API smoke, all primary screen UAT, deep-link reload, and clean browser console verified |
