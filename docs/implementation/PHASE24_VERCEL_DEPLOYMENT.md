...
The API adapter exposes `/health` through a Vercel Node Function. Authenticated routes require `AUTH_JWKS_URL`, `AUTH_ISSUER_URL`, and `AUTH_AUDIENCE` in Vercel environment variables. The adapter now loads the seeded PostgreSQL snapshot at cold start and persists project creation through `ProjectRepository`; financial write persistence still requires repository wiring before this is a real staging release.
