...
The API adapter exposes `/health` through a Vercel Node Function. Authenticated routes require `AUTH_JWKS_URL`, `AUTH_ISSUER_URL`, and `AUTH_AUDIENCE` in Vercel environment variables. The adapter now loads the seeded PostgreSQL snapshot at cold start; write transaction persistence still requires repository wiring before this is a real staging release.
...
3. Complete repository-backed write transactions, then run `scripts/staging-preflight.ps1` and `scripts/staging-verify.ps1` against the real staging API.
